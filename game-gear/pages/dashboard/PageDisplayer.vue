      ...
      <v-dialog v-model="dialog" max-width="295">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="red" text @click="deletePage()">
              Delete
            </v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  watch: {
    async page () {
      this.loading = true;
      const res = await api.getStructures(this.page.pageName);
  ...
  methods: {
    async deletePage () {
      ...
      const res = await api.deletePage(this.page.pageID);
      if (res.status === 202) {
        this.dialog = false;
        this.page = null;
      } else {
    ...
    async saveNewName () {
      if (
        ...
        const req = await api.editPageName(this.page.pageID, this.newPageName);
