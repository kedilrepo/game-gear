<template>
  <v-layout>
    <v-col align="start">
      <v-row>
        <v-spacer></v-spacer>
        <v-btn color="#009688" @click="addingUser = true">Add User</v-btn>
      </v-row>
      <div class="userTitle">
        <v-row>
          ...
          <v-spacer />
          <v-btn color="#009688" @click="reload()">Reload</v-btn>
        </v-row>
      </div>
  ...
  methods: {
    ...
    async addUser() {
      if (
        ...
        let req = await api.addUser(this.newUserId);
